import {
  ...
  type Schema as GenerativeAIFunctionDeclarationSchema,
  ...
  implements GoogleGenerativeAIChatInput
{
  ...
  constructor(
    ...
    const fields =
      typeof modelOrFields === "string"
        ...
        : modelOrFields;
    super(fields);
    this._addVersion("@intrvls/langchain-google-genai", __PKG_VERSION__);
    ...
    // Gemini 3+ models can enter infinite loops / degrade with `temperature` <
    // 1.0, so default to 1.0 when the caller didn't set one explicitly. Mirrors
    // langchain_google_genai's `validate_environment`.
    if (fields.temperature === undefined && isGemini3OrLater(this.model)) {
  ...
  invocationParams(options?: this["ParsedCallOptions"]): GenerateContentConfig {
    ...
    return {
      ...
      // `GoogleGenerativeAIThinkingConfig` exposes `thinkingLevel` as a loose
      // string union for ergonomics, whereas the SDK types it as a (nominal)
      // `ThinkingLevel` enum, so we cast at the boundary.
      ...(this.thinkingConfig
  ...
  async _generate(
    ...
    if (res.usageMetadata) {
      usageMetadata = convertUsageMetadata(res.usageMetadata, this.model);
    }

    const generationResult = mapGenerateContentResultToChatResult(res, {
      usageMetadata,
    });
  ...
  async *_streamResponseChunks(
    ...
    for await (const response of stream) {
      ...
      if (
        ...
        const candidatesDelta = Math.max(
          0,
          newCandidatesTokenCount - prevCandidatesTokenCount
        );
        prevCandidatesTokenCount = newCandidatesTokenCount;

        // Reasoning/thinking tokens are reported cumulatively too; deltize them
        // and fold into `output_tokens` so chunk totals sum correctly.
        ...
        usageMetadata.output_token_details = reasoningDelta
          ? { ...usageMetadata.output_token_details, reasoning: reasoningDelta }
  ...
  >(
    ...
    if (method === "functionCalling") {
      ...
      const tools: GoogleGenerativeAITool[] = [
        {
