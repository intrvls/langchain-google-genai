...
test("Stream token count usage_metadata", async () => {
  const model = new ChatGoogleGenerativeAI({
    ...
    model: "gemini-2.5-flash",
    maxOutputTokens: 10,
    // gemini-2.5-flash thinks by default; with a 10-token cap the entire
    // budget is spent on thinking and the API streams no content (and thus no
    // usage). Disable thinking so the short response is actually produced.
...
describe("GoogleSearchRetrievalTool", () => {
  test("Supports GoogleSearchRetrievalTool", async () => {
    const searchRetrievalTool: GenerativeAITool = {
...
describe("CodeExecutionTool", () => {
  test("Supports CodeExecutionTool", async () => {
    const codeExecutionTool: GenerativeAITool = {
  ...
  test("CodeExecutionTool contents can be passed in chat history", async () => {
    ...
    // With the code-execution tool bound, the model may reply with either a
    // plain string or structured content parts; assert on the extracted text.
