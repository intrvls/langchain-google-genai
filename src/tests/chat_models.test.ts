...
describe("Gemini 3+ default temperature", () => {
  test("defaults to 1.0 for gemini-3+ models when not set explicitly", () => {
    for (const model of [
      ...
      const m = new ChatGoogleGenerativeAI({ model, apiKey: "test-key" });
      expect(m.temperature).toBe(1.0);
    }
  });

  test("respects an explicit temperature on gemini-3+ models", () => {
  ...
  test("leaves temperature unset for pre-gemini-3 models", () => {
    const m = new ChatGoogleGenerativeAI({
...
describe("API key environment fallback", () => {
  test("falls back to GEMINI_API_KEY when GOOGLE_API_KEY is unset", () => {
    vi.stubEnv("GOOGLE_API_KEY", undefined);
    vi.stubEnv("GEMINI_API_KEY", "gemini-env-key");
    try {
      const m = new ChatGoogleGenerativeAI({ model: "gemini-2.5-flash" });
      expect(m.apiKey).toBe("gemini-env-key");
    } finally {
      vi.unstubAllEnvs();
    }
  });
  ...
  test("prefers GOOGLE_API_KEY over GEMINI_API_KEY", () => {
    ...
    try {
      const m = new ChatGoogleGenerativeAI({ model: "gemini-2.5-flash" });
      expect(m.apiKey).toBe("google-env-key");
    } finally {
      vi.unstubAllEnvs();
    }
  });
...
test("Google AI - passes system instruction and response schema per request", async () => {
  ...
  vi.spyOn(model, "completionWithRetry").mockImplementation(async (request) => {
    requests.push(request as GenerateContentParameters);
    return mockGenerateContentResponse("ok") as never;
  });
  ...
  expect(requests[0].config?.responseSchema).toBe(responseSchema);
  expect(requests[0].config?.responseMimeType).toBe("application/json");
  expect(requests[1].config?.systemInstruction).toBeUndefined();
  expect(requests[1].config?.responseSchema).toBeUndefined();
  expect(requests[1].config?.responseMimeType).toBeUndefined();
});
